...
```json
{
  ...
  "permissions": [
    ...
    "core:window:allow-current-monitor",// 允许获取当前显示器信息
    "core:window:allow-show",// 允许显示窗口
    "core:window:allow-hide",// 允许隐藏窗口
    "core:window:allow-set-focus",// 允许设置窗口焦点
    "core:window:allow-set-ignore-cursor-events"// 允许设置忽略鼠标事件
  ]
}
```
原理说明：
...
Tauri 提供了一个强大的 API 解决鼠标穿透：`setIgnoreCursorEvents`。
- `true`: 窗口忽略鼠标事件（点击会穿透到下层）。
- `false`: 窗口正常接收鼠标事件。
...
"permissions": [
  // ... 其他权限
  "core:window:allow-set-ignore-cursor-events",// 允许设置忽略鼠标事件
  "core:window:allow-outer-position" // 获取窗口位置需要权限
]
```

#### 全`KeyVisualizerWindow.vue`代码：
```vue
...
## 6. 组合键算法
当收到 `KeyPress` 事件时：
1.  **如果是修饰键 (Ctrl/Shift/Alt/Win)**:
    - 加入 `activeModifiers` 集合。
    - 标记为“未使用” (dirty = false)。
    - **不立即显示**。
2.  **如果是普通键 (A/B/C)**:
    - 立即显示！
    - 显示内容 = `当前所有修饰键` + `当前普通键` (例如 "Ctrl + Shift + A")。
    - 标记修饰键为“已使用” (dirty = true)。
3.  **如果是修饰键松开 (KeyRelease)**:
    - 如果该修饰键从未被组合使用过 (dirty == false)，说明用户只是单按了一下 Ctrl，此时才显示 "Ctrl"。
    - 从集合中移除。